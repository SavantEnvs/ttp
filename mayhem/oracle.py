...
sample (mayhem/fuzz-ttp/testsuite/seed.data parsed with mayhem/fuzz-ttp/testsuite/seed.template). A
no-op / neutered program (which prints nothing) FAILS test.sh, because the SELFTEST_PASS marker and
its asserted values are only printed when every assertion holds.
"""
import pathlib
...
# seeds are at <repo>/mayhem/fuzz-ttp/testsuite/* (the canonical per-target seed path; the whole repo
# is COPYed to /mayhem in the commit image).
HERE = pathlib.Path(__file__).resolve().parent
TEMPLATE = (HERE / "fuzz-ttp" / "testsuite" / "seed.template").read_text()
DATA = (HERE / "fuzz-ttp" / "testsuite" / "seed.data").read_text()
